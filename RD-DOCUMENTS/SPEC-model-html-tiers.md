...
## Tier 2m — a mark you can see, positioned by a wall you can't (6 Sep)

Not a painter. The defect tier 2l carried across faithfully, and the drawing
Movie used to kill it.

### The porch

> A 30 x 40 bungalow with a covered front porch. On FOUNDATION you draw a frost
> wall 8 ft out in front of the house to carry the porch posts — correct
> drawing, that is where the concrete goes. Switch back to MAIN FL and the
> porch foundation is not shown, but **E1 has moved 8 ft into the front yard.**

`planWallExtents` filtered `wall.levelId > 0` and nothing else: every wall on
every level and every layer view. `eMarkDimEdges` had the same blindness on
dimensions. Movie's question settles it — *"should a mark you can see be
positioned by a wall you can't?"* — and the answer is no.

**It was never only the ring.** The same box feeds `cutLineSpan`, so the porch
wall stretched hand-placed section lines too, running them 8 ft past the front
wall before the bubble landed. One filter, two symptoms, and the second is the
one nobody would have gone looking for.

### Both halves, or it looks fixed and isn't

Filtering the walls and not the dimensions leaves a string on a hidden layer
view still pushing the marks out, on a drawing where nothing visible is out
there. Half a fix here is worse than none because it presents as a whole one.

### The caller decides, and the module stops guessing

The fix is not a view parameter threaded through four functions. **The caller
hands over what it is showing** — `_activeWalls()` / `_activeDimensions()` on
the bone, `walls()` / `dimensions()` on MODEL.html — and `cut-marks.js` keeps
no filter of its own.

That is the point rather than a convenience: those accessors already know about
levels, layer views and the courtesy escape across a switch, in one place. Two
filters, one of them invisible from the call site, is how the porch bug
survived. A `levelId > 0` kept in the module would now be dead against both
callers while reading as though it guarded something.

### What deliberately did NOT change

**The stored drag.** `elevationMarkOffsets` is keyed by mark id and not by
view, and Skipper flagged that one offset would now be applied to bases that
differ per view. Checked rather than reasoned about: the drag writes
`clampEMarkClear(sign * (raw[axis] - base))` — **a clearance from the edge, not
a position** — and `MODEL.dc.html:2761` says so in as many words, *"per-drawing
E1-E4 clearances from the outermost dim string"*. A correct clearance off a
correct base is a correct position. The mark does land at different coordinates
per view; that is the stored meaning being honoured. Decided, with evidence,
rather than discovered later.

**The mark's length.** Movie, 6 Sep: *"the second one — never clip the
elevation, leave the line."* The auto marks keep full plan width plus the 2 ft
pad. "Shown inward" was about the elevation never being clipped, which belongs
to the elevation generator, not here.

**The 2 ft clearance and the per-side placement**, which already matched what
Movie described. No redesign — one filter.

### The proof is a test that stopped needing a workaround

Tier 2l had to switch the elevation ring OFF for the whole of
`tests/model-html-joins.spec.js`: the moving ring broke its equality test and,
worse, made its two INEQUALITY tests pass for the wrong reason.

**That containment is removed and the file passes with the ring on.** The
harness can only show the module ignores what it is not handed; only the page
test can show the page does not hand it over. The failure that started this is
the thing certifying the cure.

### THE HABIT, and it earned its name three times in one evening

Every one of these was a whole-file search finding a true string in the wrong
scope:

| | what was searched | what it actually was |
|---|---|---|
| tier 2j | a hex at `render-2d.js:728`, called "underlays'" | `drawOrigin2D`'s line — `drawUnderlays2D` has no colour at all |
| tier 2l | `normaliseStructureStandards` in an export list | on `DraftStructureStandards`, not `DraftProfileManager` |
| here | `walls: this._walls,` | `drawFixture2D`'s env, where it is CORRECT |

The third one nearly had me "fix" working code: `tubGeometry` scans for the
wall that closes the alcove and does its own filtering while it scans, so the
fixture painter wants every wall. A check that cannot tell one painter's env
from another's reports a bug in code that is right.

**A search tells you a string is present. It does not tell you what encloses
it, and the enclosure is usually the claim.** Same family as the line-range
trap, and the same remedy: find the boundaries of the thing you are standing
in before believing a hit inside it. Both checks added here brace-match rather
than grep, and one of them exists only because the first version went red about
a delegation that plainly delegated.

### The bill

| | |
|---|---|
| `cut-marks.js` | two filters removed, contract stated |
| `MODEL.dc.html` | four call sites pass the active sets |
| `MODEL.html` | `paintCuts` passes `walls()` / `dimensions()` |
| `tests/model-html-joins.spec.js` | the tier 2l containment removed — the end-to-end proof |
| checks | **59 checks, 24 mutations**, including the porch in both symptoms |
