...
| **NEW-6** | **MODEL.html tier 3 — the page becomes a drafting surface.** The ladder, and it is the live one: **3a is DONE, four rungs of four** (select `ec993b3`, corner `4e2be48`, save `75cc4ac` + close guard `be01325`, whole wall `a0c3841`, PRs #346 / #347), plus the store rung it forced on the old page (`8992ad9`, the stale write becomes a refusal instead of an eat). **Updated 13 Sep — six more rungs have landed and this row described none of them.** **3b** (`0e018c2`, level AND layer-view switchers, keyed by id through `?level=`) and **3c** (`435778e`, draw, delete, undo either) are **DONE**. So are the cut-view rungs: the **host** (`8eeb9f4`, a cut reached by `?view=cut:<id>`, extending 3b rather than growing a second source of truth), the **six seats** (`aa065cb` + `93f41ff`, one `allCuts()` list feeding picker, rail, readout and lookup so none can disagree), and **live elevation thumbnails** (`76ab2e0`, `coarseSilhouette` — full-size views and LAYOUT's printed sheets untouched). **In flight:** the chrome shell (Skipper — top bar, two collapsible sidebars, a properties slot) and the seven small tools' data survival (Gilligan — preserved / painted / linked, measured before any palette is built). **The ladder lives in `SPEC-model-html-tiers.md`, not here, and the rule there is one entry per rung as it lands, with the commit.** This row exists so the board says tier 3 is running at all: it ran for five days while the spec's entire tier-3 section read *"not specced here"* and this file said nothing — and then this row went stale by six rungs in four days, which is the same failure one level up. **Do not plan a switcher off `activeLevelIdx`** — it is an index, `MAIN_FLOOR_LEVEL_ID` is an id, and they agree on every default drawing and disagree the moment a level is inserted; 3b avoided it rather than survived it. **What the page still cannot do:** no redo, no add/insert/delete level, no change-wall-type, no tool palette, and eighteen of the nineteen old-page tool states unreachable. | shell + tools: the bulk of what is left | Nothing waits on it. It is the tier that decides whether the new page replaces the old one or stays a viewer — and `MODEL.dc.html` cannot be unlinked, let alone deleted, until it closes. |
