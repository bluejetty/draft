...
|---|---|---|---|---|
| Copy one object's properties onto another | **absent** — a dimmed chip with no handler (`MODEL.dc.html:2009`) | **present** — the DRAFTING BRUSH | done | THE FIRST ROW THAT RUNS THE OTHER WAY. Everywhere else on this table the old page is the bar and this one is catching up; here the old page has a picture of a tool and nothing behind it, and has had since before this page existed. `AUTOWINDOWS-PR-NOTES.md` named it as left out on purpose -- "no eyedropper / fenestration property painter -- later slice of #169" -- and `SPEC-drafting-brush.md` designed it with Movie on 5 Sep, leaving two questions marked "Needs Movie". He answered both on 20 Sep: EXACT properties, onto a MATCHING kind only ("a WALL will only change another WALL"), a window under the cursor CHANGES while bare wall gets a new opening PLACED, and a load that will not fit is refused with TOO BIG rather than accepted. **The rule that settles every edge case** is the spec's: the brush carries what a thing IS, never where it is -- so `BRUSH_PROPS` lists type, size, head, sill and layer, and the keys it leaves OUT (offset, wallId, points, levelId) are the substance of it. **It adds no capability**, which is why no absence row moved with it: changing an existing object is the PROPERTIES box and placing an opening is the fenestration tool, both already here. **What it cannot reach is said rather than discovered** -- Movie named DIMENSIONS, and `hitAt`'s chain is fenestration, wall, line, outline, floor, so a dimension cannot be pressed on this page at all. Columns and beams are the same story from the other end: selected by being placed, never found again by a press. **The load outlives the chip** -- arming a drawing tool puts the brush's press down and keeps what it holds, so the next wall comes out wearing it |
| Draw a wall | wall tool | **present** — survives a reload | must-have | **driven**; the mutation row: `drawPress` no-op makes this fail |
| Draw a line | line tool | absent | must-have | DRAWING one is still absent. **Reading one is not**: a line can be selected, and LINE PROPERTIES says how long it is and that a line is two points — which is the whole of its record besides a level, a view and a layer. An empty box would have read as "this page has no properties for a line", which is not what is true |
| Draw a floor | floor tool | **present** — corners in, close on the first | done | the row read "it paints floors it cannot create" from tier 2c, when `drawFloor2D` was wired in: a floor drawn by the old page arrived here and rendered, corner handles and all, and a drafter sitting in front of this page had no way to make one. **The gesture is the outline's, deliberately** — press corners, close on the first — because that is the old page's floor gesture too (`_handleFloorClick`), so a drafter who has learnt one has learnt both; the state is a separate list from the outline's, since an abandoned trace of either must not finish as the other. **What the spec measures is the record**, and one pair of fields above the rest: `view` and `structure`. A floor saved with the wrong view is INVISIBLE — it lands on a layer set the drafter is not looking at — and a framed floor saved as a slab is three inches of concrete where a foot of joists belongs; neither shows on the canvas the moment it is drawn. Both come from `layer-views.js`'s `floorHomeView` rather than a rule written here, and its surprising clause is the one under test: a level that HAS a foundation set files its floor there WHICHEVER set it was drawn on, because such a level's floor IS its foundation. The old page softens the consequence with a courtesy set (`_courtesyFloorIds`) that keeps such a floor on screen until the layer set changes; this page has no courtesy set, so it **says** where the floor went instead — a silent vanish is the one outcome that reads as the tool having failed. **One deliberate divergence**: a loop that crosses itself is REFUSED here and is not on the old page, whose `_finishFloor` checks the corner count and nothing else — a bowtie commits there and renders as two triangles with a fill that fights itself. The outline tool on this page already refuses one, and a page where the same motion is legal for a guide and illegal for structure would be teaching two rules for one gesture. The refused corners are KEPT, so the drafter moves the last one rather than retracing the room. The thicknesses are derived, not typed: a slab takes its level's `slabThicknessIn`, a framed floor `DEFAULT_FLOOR_THICKNESS_IN`, both from `level-assembly.js`. The half-drawn trace paints through `drawFloor2D`'s own `preview` mode — dashed edge, no handles — plus the two marks that painter will never make: the FIRST corner (it returns early under two points) and the close target. Out of scope and absent rather than half-done: the typed-length ruler, arc edges, arc edges and the FLOOR JOISTS box — each is a panel of its own and none of them is the gesture. **THE SLOPE AND THE THICKENED EDGE ARE NO LONGER ABSENT**: both are rows in FLOOR PROPERTIES, reached by selecting a floor, with the thickness that used to be derived at creation and never editable after. Movie asked for the second by name, 18 Sep: "i'd like the roof and thickened edge done". STRUCTURE is READ and not written there, and the reason is this row's own surprising clause: framed or slab is `layer-views.js`'s answer from where the floor LIVES, not a property of its own — a picker would let a drafter put a slab on a plan set and make the two answers disagree |
...
| Change a wall's type | wall-type picker on a group | **present** — and on a WALL, not a group | done | #377 established there is no change-type verb at all; there is one now, and it is NOT the old page's shape. `_applyGroupWallSettings` reads `_selectedGroupForWallEditing()`, so changing one wall's type there means bundling it into a named ASSEMBLY first. Movie ruled otherwise for this page, 19 Sep: "if they select a specific wall or multiples they can change the properties". The RECORD is the old page's, field for field (`:18110-18118`) — wallType, refLine, baseHeight, topHeight — including the clause that **a drafter's change ends the wall-type substitution**, without which a wall just set to a new assembly saves as the retired type it was loaded from. The types offered are the CONTEXT'S (`contextWallTypes`), because foundation and framed assemblies are disjoint lists and offering an 8" pour for a main-floor wall is a wall of the wrong kind rather than a near miss. A selection that DISAGREES lights no chip and shows an empty field: a lit chip is a claim about every wall in the hand. Heights are typed architectural lengths through the shared parser, refused in place, and refused PER WALL so a selection with different tops cannot slip one through. **RAIL joined `wall-types.js`** with it (Movie: "lets consider a 'RAIL' a wall type"), so a pony wall is a wall whose type is RAIL and needs no key of its own — the rail's own properties, balusters and the pony height behaviour are NOT here. Still absent: changing the storey DEFAULT so the walls still sitting on it follow, which is `PROJECT.html`'s to own |
...
| PROPERTIES box | per-tool rows beside the drawing | **present** — left rail, under the tool column, with a fold of its own | done | Movie, 19 Sep: "we should designate an area on the model page for the PROPERTIES box (which will change depending on what you are using) i'm thinking add a collapsable box on the left side under the DRAFTING TOOLS". It sat under LEVELS / LAYERS on the RIGHT until then, which put the thing you change a wall with on the far side of the sheet from the thing you draw a wall with. It FOLLOWS THE SELECTION now and falls back to the armed tool when nothing is selected — `SELECTION_PANELS` is the table the object panels join, and OPENING, FLOOR and LINE have. **ROOF has not, and it is BLOCKED rather than outstanding**: `hitAt`'s chain is fenestration, wall, line, outline, floor — a roof cannot be selected on this page at all, so a panel for one would have nothing to open it. Putting roofs in the pick chain is its own rung; the roof TOOL's overhang and pitch box is a different question, about the NEXT roof. Its fold is its own, not the rail's, because the tool column above it is six rows tall; the state rides in the URL beside `left` and `right`. **It closed a defect nobody had reported**: the column/beam panel was a watcher that RETURNED EARLY when the selection was not its kind and left the old panel standing, so a press on a wall left COLUMN properties on screen with the column deselected |
