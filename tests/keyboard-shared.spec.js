...
test('Model Space labels its tools from the shared defaults', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: new RegExp(`Line\\s+\\[${shared.line}\\]`) })).toBeVisible();
  await expect(page.getByRole('button', { name: new RegExp(`Wall\\s+\\[${shared.wall}\\]`) })).toBeVisible();
});
