...
async function placeCut(page, name, z) {
...
// the concrete grays of the foundation walls / footings / slab.
async function overlayCensus(page) {
  return page.evaluate(() => {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      else if (r > 185 && r < 218 && g > 185 && g < 218 && b > 185 && b < 220) concrete += 1;
    }
    return { ink, concrete };
  });
}
...
test.describe('Generated section view', () => {
  ...
  test('a cut across empty space explains itself instead of drawing', async ({ page }) => {
    ...
    expect(census.concrete).toBeLessThan(1000);
