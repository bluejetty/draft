...
| --- | --- | --- | --- |
| **NEW-2** | **Note A — composition order.** Add site, roof, basement and electric sheets so the dealt set matches the approved sheet order. Also switch the scale ladder to **try 3/16" first and step only down, never up** — today it deals elevations at 1/16" and they swim on an 11×17 (Movie, 1 Sep; a wide house lands on 1/8", a narrow one holds 3/16"). **Fit must be measured on the annotated extent, not the building envelope** — dimension strings, elevation/level markers and the view title all sit outside the object, so a set that fits bare comes back one step too big the day dimensions land. | 2–4 h | The whole default-set feature reads as half-finished until this lands. |
