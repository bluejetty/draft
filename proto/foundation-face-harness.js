...
// ── THE SILL BAND: WHAT NOTHING WAS WATCHING ────────────────────────────
//
// Movie, 25 Sep, on his own 2 STOREY + GARAGE + ROOM OVER: *"the garage at
// floor level doesn't connect properly still ... (main floor level)"*, and
// then the rule it has to satisfy:
//
//   "1.5" sill plate is under the main floor (which lines up with the garage
//    sill plate) and under the sill plate is 8' height foundation on house
//    side / 32" grade beam under the sill plates on the garage side"
//
//   "the garage sill CAN'T go higher than the house sill"
//
// Measured on his file: main floor to garage floor read 11.100" where his
// framing -- 11-7/8" of joist plus 3/4" of sheathing -- makes it 1'-0-5/8".
// The 1.5" of error was the plate, counted twice on the garage and not at
// all on the house, because `assemblyFor(1).wallHeightFt` is 8'-1.5" and two
// places read it as the top of CONCRETE.
//
// WHY THESE CHECKS EXIST AT ALL. Fixing it moved the whole foundation down
// 1.5" -- and 61 harnesses and 101 foundation, footing, garage, section and
// premade specs passed through that move without one of them noticing.
// Nothing in the repo pinned where the foundation sits vertically. The green
// was real and it was worth almost nothing, which is the gap this closes.
// WHICH OF THESE BITE ON WHAT, because the fixture is a build made AFTER the
// fix and that limits what it can prove. Measured both ways:
//
//   the two STACK checks bite on the source. Reverted, they read
//   "wallTop -1.0521 vs floorBottom -1.0521 less 1.500\"" and
//   "11.125\" vs 12.625\"" -- Movie's defect in Movie's own numbers.
//
//   the RECORD checks bite on the builder, and cannot be shown to from this
//   fixture: its garage already tops at 8. Against his pre-fix file the same
//   assertions read house=8 garage=8.125, so "never ABOVE" is false and
//   "level with it" is false. They guard `raiseGarageConcrete` from raising
//   the beam again; they do not re-prove it was ever raised.
const SILL_PLATE_FT = 1.5 / 12;
const BAND = path.join(ROOT, 'proto', 'repro-sill-band.draft');
if (!fs.existsSync(BAND)) {
  failures.push('proto/repro-sill-band.draft is missing');
} else {
  const saved = JSON.parse(fs.readFileSync(BAND, 'utf8'));
  const env = H.buildEnv(win, saved);
  const CV = win.DraftCutView;
  const stack = CV.sectionLevelStack(env);
  const main = stack.floors.find(f => f.name === 'MAIN FL');
  const near = (a, b, tol = 1e-6) => Math.abs(a - b) < tol;
  const inches = v => `${(v * 12).toFixed(3)}"`;

  // THE BAND EXISTS. This is the one that does not need a building: the top
  // of concrete stands a plate's thickness under the floor it carries, and
  // before the fix it stood hard against the joists with no room for one.
  check('the top of concrete stands one sill plate under the main floor',
    near(stack.foundation.wallTop, main.floorBottom - SILL_PLATE_FT),
    `wallTop ${stack.foundation.wallTop.toFixed(4)} vs floorBottom `
    + `${main.floorBottom.toFixed(4)} less ${inches(SILL_PLATE_FT)}`);

  // AND THE DROP IS THE FLOOR ITSELF. With both sills in one band, what
  // separates the two floors is exactly the assembly between them -- which
  // is why Movie could give the number from his framing alone.
  const drop = main.floorTop - (stack.foundation.wallTop + SILL_PLATE_FT);
  check('main floor to garage floor is the main floor assembly, and nothing else',
    near(drop, env.levelFloorFt(main.id), 1e-6),
    `${inches(drop)} vs ${inches(env.levelFloorFt(main.id))}`);

  // THE TWO CONCRETE TOPS, off the records rather than the ink: a build
  // whose garage and house disagree here cannot have level sills whatever
  // the painter does with them.
  const lvl1 = (saved.walls || []).filter(w => Number(w.levelId) === 1);
  const houseTops = [...new Set(lvl1.filter(w => w.body !== 'garage').map(w => w.topHeight))];
  const garageTops = [...new Set(lvl1.filter(w => w.body === 'garage').map(w => w.topHeight))];
  check('every house foundation wall tops out at one height', houseTops.length === 1,
    houseTops.join(','));
  check('so does every piece of the garage-s', garageTops.length === 1, garageTops.join(','));
  if (houseTops.length === 1 && garageTops.length === 1) {
    // MOVIE'S RULE IS AN INEQUALITY, not an equality, and it is written that
    // way on purpose: a FROST WALL garage sits GARAGE_SILL_BELOW_HOUSE_FT
    // lower and is still correct. What is never correct is higher.
    check('the garage sill is never ABOVE the house sill',
      garageTops[0] <= houseTops[0] + 1e-9,
      `garage ${garageTops[0]} vs house ${houseTops[0]}`);
    // On a grade beam it is level -- Movie: "at this position it matches the
    // house sill so this is the highest it can go".
    check('and on a grade beam it is level with it',
      near(garageTops[0], houseTops[0]),
      `garage ${garageTops[0]} vs house ${houseTops[0]}`);
    check('the grade beam is still 32 inches of concrete',
      near(garageTops[0] - lvl1.find(w => w.body === 'garage').baseHeight,
        CV.STANDARDS.GARAGE_BEAM_CONCRETE_IN / 12),
      inches(garageTops[0] - lvl1.find(w => w.body === 'garage').baseHeight));
  }

  // AND GRADE MOVED WITH THE CONCRETE, which is the visible side effect and
  // belongs in a check rather than in a commit message alone. cut-view's own
  // comment says grade is 1'-2" below "the top of the foundation wall" and
  // that "this file's 'foundation top' IS the top of concrete"; it had been
  // measuring from the plate's top instead.
  check('grade is measured from the top of CONCRETE, as its own comment says',
    near(stack.foundation.grade,
      stack.foundation.wallTop - CV.STANDARDS.GRADE_BELOW_FOUNDATION_TOP_FT),
    `grade ${stack.foundation.grade.toFixed(4)} vs wallTop `
    + `${stack.foundation.wallTop.toFixed(4)} less `
    + `${CV.STANDARDS.GRADE_BELOW_FOUNDATION_TOP_FT.toFixed(4)}`);
}
