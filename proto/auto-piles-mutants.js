...
const MUTANTS = [
  ...
  // floor is over the garage". Both marks are 12" DIA, so nothing DRAWN moves
  // and only these can fail. The share threshold gets a mutant at each end,
  // because the two designs sit at ~3% and ~70% and a rule that answered
  // either extreme would look entirely reasonable in the source.
  { file: 'MODEL.html',
    name: 'any overlap counts, so the garage tie alone reads as a room over',
    find: '  const ROOM_OVER_SHARE = 0.25;',
    with: '  const ROOM_OVER_SHARE = 0;',
    test: 'a two storey whose upper floor stops at the house keeps P2' },
  { file: 'MODEL.html',
    name: 'only a garage covered end to end counts as built over',
    find: '  const ROOM_OVER_SHARE = 0.25;',
    with: '  const ROOM_OVER_SHARE = 1.5;',
    test: 'a room over the garage takes P3, not P2' },
  { file: 'MODEL.html',
    name: 'the mark is read off the garage\'s OWN storey, not the one above it',
    find: '    const above = storey ? floorCarriedBy(storey.id) : null;',
    with: '    const above = storey;',
    test: 'a room over the garage takes P3, not P2' },
  { file: 'MODEL.html',
    name: 'the two marks are swapped',
    find: "  const GARAGE_PILE_MARK_ONE_STOREY = 'P2';\n  const GARAGE_PILE_MARK_ROOM_OVER = 'P3';",
    with: "  const GARAGE_PILE_MARK_ONE_STOREY = 'P3';\n  const GARAGE_PILE_MARK_ROOM_OVER = 'P2';",
    test: 'a room over the garage takes P3, not P2' },
  { file: 'MODEL.html',
