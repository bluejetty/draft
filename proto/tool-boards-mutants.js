...
const MUTANTS = [
  ...
  { file: 'MODEL.html',
    ...
    find: '  boardListeners.push(() => { setTool(activeTool); buildToolColumn(); });',
    with: '  boardListeners.push(() => { buildToolColumn(); });',
    test: 'puts down a tool TOY does not offer' },
  { file: 'MODEL.html',
    name: 'a board change disarms EVERYTHING, including what the board offers',
    find: '  boardListeners.push(() => { setTool(activeTool); buildToolColumn(); });',
    with: '  boardListeners.push(() => { setTool(RESTING_TOOL); buildToolColumn(); });',
